...
const PROVIDERS: Array<{
  ...
  },
  {
    id: 'facebook',
    label: 'Facebook',
    tagline: 'Publica videos en tu página o perfil',
  ...
  },
  {
    ...
    connectLabel: 'Iniciar sesión con TikTok',
  ...
  },
  {
    ...
    connectLabel: 'Iniciar sesión con YouTube',
...
const STEP_LABELS = ['Conecta tus redes', 'Selecciona contenido', 'Publica en 1 clic'];
...
export default function AccountsPage() {
  ...
  const allConnected = connectedSet.size === 4;
    ...
    <main className="relative mx-auto w-full max-w-6xl space-y-6 px-4 py-6 sm:px-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm text-muted-foreground">
            Inicia sesión en cada red una sola vez. Después sube videos desde tu PC y publícalos en todas a la vez.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:border-brand-cyan/50 hover:text-brand-cyan transition-colors"
          >
            <Loader2 size={14} className={cn(refreshing && 'animate-spin')} />
          </button>
        </div>
      </header>
        ...
        {STEP_LABELS.map((label, i) => (
          <li
            ...
            className={cn(
              ...
              i === 0
                ? 'border-brand-cyan/50 bg-brand-cyan/10 text-white shadow-glow-cyan'
          ...
          >
            <span
              className={cn(
                ...
                i === 0
                  ? 'bg-gradient-to-br from-brand-cyan to-brand-violet text-white'
              ...
              <div className="mt-4 flex-1 space-y-2">
                <AnimatePresence initial={false}>
                  {list.map((account) => (
                    ...
                    >
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-brand-cyan to-brand-violet text-[10px] font-bold text-white">
                ...
                {!loading && list.length === 0 && (
                  <p className="rounded-lg border border-dashed border-border px-3 py-2 text-center text-[11px] text-muted-foreground">
                    Aún no hay cuentas de {provider.label} aquí
                  </p>
                )}
              </div>
        ...
        <section className="glass neon-border relative overflow-hidden rounded-2xl p-6 sm:p-8">
          ...
          <div className="relative flex flex-col items-start justify-between gap-5 sm:flex-row sm:items-center">
            <div className="flex items-start gap-4">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-cyan to-brand-violet shadow-glow-cyan">
                <HardDriveUpload size={24} className="text-white" />
              </span>
              <div>
                <h2 className="text-lg font-bold text-white sm:text-xl">
                  Conecta {connectedSet.size} de 4 —{' '}
                  <span className="text-gradient">sube tus videos ahora</span>
                </h2>
                <p className="mt-1 max-w-lg text-sm text-muted-foreground">
                  Elige MP4, WEBM o MOV desde tu PC. Se encolan solos para Instagram, YouTube, Facebook y TikTok.
                </p>
              </div>
            </div>
            <Button
      ...
      {connectedSet.size > 0 && (
        <p className="flex items-center justify-center gap-2 text-center text-sm text-muted-foreground">
          ...
          Todo listo para publicar — ve a <span className="font-semibold text-foreground">Contenido</span> y lanza tus videos a las redes conectadas.
        </p>
      )}

      <UploadModal open={uploadOpen} onClose={() => setUploadOpen(false)} onUploaded={() => void fetchAccounts(true)} />
    </main>
  );
}
