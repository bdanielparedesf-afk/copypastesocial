/** FASE 12 - OAuth de YouTube (Google Identity Services).
...
/** Genera un verifier y challenge para PKCE. Returns [verifier, challenge]. */
...
export async function refreshYouTubeToken(refreshTokenValue: string, _currentAccessToken?: string): Promise<{
  access_token: string;
  expires_in: number;
}> {
  const body = new URLSearchParams(Object.entries({
...
/** Encrypta un token usando la key de encriptación. */
